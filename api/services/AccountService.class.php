...
class AccountService extends BaseService
{
    ...
    public function register($account)
    {
        if (!isset($account['username'])) {
            throw new Exception("Account field is required");
        }
    ...
    public function login($account)
    {
        ...
        $jwt = \Firebase\JWT\JWT::encode(["exp" => (time() + Config::JWT_TOKEN_TIME), "id" => $db_account["id"], "role" => $db_account["role"]], "JWT SECRET");
    ...
    public function forgot($account)
    {
        ...
        if (strtotime(date(Config::DATE_FORMAT)) - strtotime($db_account['token_created_at']) < 300) {
            throw new Exception("Be patient! Token is on his way.", 400);
        }
    ...
    public function reset($account)
    {
        ...
        if (strtotime(date(Config::DATE_FORMAT)) - strtotime($db_account['token_created_at']) > 300) {
            throw new Exception("Toxen has expired", 400);
        }
